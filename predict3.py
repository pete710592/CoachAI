...
video = 'NUTN_' + videoName.split('NUTN_')[1]
video_dir = videoName.split('NUTN_')[0]
predict_dir = "predict"

# Get badminton machine parameters
parameters = video.replace(".mp4", "")
...
predictCSV_path = os.path.join(predict_dir, (video[:-4]+'_predict.csv'))
f = open(predictCSV_path, 'w')
f.write('Frame,Visibility,X,Y,Time,Position,Height,Speed,Angle_H,Angle_V\n')
...
if video[-3:] == 'avi':
	...
	exit(1)
predictMP4_path = os.path.join(predict_dir, (video[:-4]+'_predict'+videoName[-4:]))
out = cv2.VideoWriter(predictMP4_path, fourcc, fps, size)
...
while success:
	...
	for i in range(3):
		...
		if np.amax(h_pred[i]) <= 0:
			f.write(str(count)+',0,0,0,'+frame_time+','+parameters+'\n')
			out.write(image)
		else:	
			#h_pred
			...
			f.write(str(count)+',1,'+str(cx_pred)+','+str(cy_pred)+','+frame_time+','+parameters+'\n')
